...
- [ ] 1.2 Add `toast` field to Zustand store (`string | null`, default null) and a `showToast(msg)` action that sets the toast and auto-clears after ~3 seconds via setTimeout
...
- [ ] 5.2 On "Close Full Position" tap: call `store.closePosition(id)`, `store.dismissOverlay()`, `store.showToast('Position closed — $X received')`
...
- [ ] 8.5 Verify toast auto-dismisses after ~3 seconds
