...
- "Start verification" button shows a loading spinner for 1.5s then navigates to 'referral'
...
3. **Simulated loading state on CTA** — Button shows Loader2 spinning icon for 1.5s via `setTimeout`, then calls `store.navigate('referral')`. Uses local React state (`isLoading`) to toggle between button text and spinner.
